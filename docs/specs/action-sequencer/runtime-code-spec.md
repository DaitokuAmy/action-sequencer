...
`SequencePlayer` は複数の `SequenceClip` を並列再生できます。
...
- local
特定 `SequencePlayer` インスタンスに対する登録。
- global
全 `SequencePlayer` から参照される静的登録。
...
`SequenceHandle` は `PlayingInfo` 参照を直接持たず、`SequencePlayer` と `playId` のみを持ちます。
...
- `IsDone`
対象 `playId` がまだ `SequencePlayer` に存在するかで判定する。
- `Stop()`
対象再生がまだ存在する場合のみ停止する。
- `IEnumerator`
...
`SequenceHandle` は value type です。コピーされても同じ `playId` を指すだけで、再生情報本体は `SequencePlayer` 側で一元管理します。
...
- Action 実行元 1 単位ごとに `SequencePlayer` を所有し、ライフタイム終了時に `Dispose()` する。
